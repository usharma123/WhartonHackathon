...
export async function validatePropertyFromExpediaUrl(
  ...
  const normalizedUrl = sourceProvider.normalizeUrl(input.expediaUrl);
  await store.patchProperty(input.propertyId, {
    sourceVendor: sourceProvider.vendor,
    sourceUrl: normalizedUrl,
    validationStatus: "refreshing",
  });

  try {
    const fetchedAt = store.now();
    const scrape = await sourceProvider.scrapeProperty(normalizedUrl);
    const snapshot = extractExpediaSnapshot(scrape, normalizedUrl);
    const signals =
    ...
    const liveReviews =
      snapshot.reviews.length > 0
        ...
        : [];
    const updatedProperty = await applyImportedSnapshot(
      store,
      {
        propertyId: input.propertyId,
        sourceVendor: snapshot.sourceVendor,
        sourceUrl: normalizedUrl,
        propertySummary: snapshot.propertySummary ?? property.propertySummary,
        popularAmenities: snapshot.popularAmenities ?? property.popularAmenities,
        city: snapshot.city ?? property.city,
        province: snapshot.province ?? property.province,
        country: snapshot.country ?? property.country,
        guestRating: snapshot.guestRating ?? property.guestRating,
        facetListingTexts: {
    ...
    const validationState = await store.getPropertyValidationState(input.propertyId);
    if (!validationState) {
      throw new Error(`Missing validation state for ${input.propertyId}`);
    }
    return {
      property: updatedProperty,
      validationState,
      sampledReviewCount: liveReviews.length,
    };
  } catch (error) {
    await store.patchProperty(input.propertyId, {
      sourceVendor: sourceProvider.vendor,
